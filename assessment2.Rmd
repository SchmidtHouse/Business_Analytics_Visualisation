...
data %>%
  filter((celebrate=="Yes") & !is.na(family_income)) %>%
  mutate(income_mean = summarise())
  ggplot(aes(family_income, gender)) +
  geom_point() +
...
random_fr_1 <- randomForest(family_income_num~celebrate+main_dish+age,data=data1, na.action=na.omit)

random_fr_2 <- randomForest(family_income_num~celebrate+us_region,data=data1, na.action=na.omit) 
